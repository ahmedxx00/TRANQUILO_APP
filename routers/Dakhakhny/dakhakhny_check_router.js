...
router.get("/lastOrder", (req, res, next) => {
  ...
  db.collection("dakhakhny_users").findOne(
    { phone: login_phone },
    (err, result1) => {
      if (err) {
        res.status(200);
        ...
        if (result1.blocked == false) {
          db.collection("dakhakhny_latest_version").findOne(
            { app_name: app_name },
            (err, result2) => {
              if (err) {
                res.status(200);
      ...
      if (err) {
        ...
        if (result) {
          ...
          if (nowTime > start_time && nowTime < end_time) {
            // go get products list
            db.collection("dakhakhny_products")
              .find({}, { projection: { _id: 0 } })
              .toArray((err, items2) => {
                if (err) {
                  res.status(200);
          ...
          db.collection("dakhakhny_polygons")
...
router.get("/ifAdminNowLiesWithinPolygon", (req, res, next) => {
  let login_phone = req.query.login_phone;
  let lat = parseFloat(req.query.lat);
  let lng = parseFloat(req.query.lng);
  let nowTime = new Date(req.query.nowTime);
  ...
  db.collection("dakhakhny_polygons").findOne(
    ...
    async (err, result) => {
      if (err) {
        ...
        if (result) {
          // admins array in polygon

          let isOneOfPolygonAdmins = Array.from(result.admins).includes(
            login_phone
          );

          if (isOneOfPolygonAdmins) {
            // assigned to polygon
            ...
            if (nowTime > start_time && nowTime < end_time) {
              ...
              res.json({
                success: "true",
                msg: "lies_and_assigned_and_now_working",
                code: code,
                end_time: end_time, // to end session when timeout
              });
            } else {
              ...
              res.json({
                success: "false",
                msg: "lies_and_assigned_but_not_now_working",
                start_time: start_time,
                end_time: end_time,
              });
            }
  ...
  db.collection("dakhakhny_gifts")
    ...
    .toArray((err, gifts) => {
      if (err) {
        ...
        db.collection("dakhakhny_feedbacks")
          .find(
            {
              read: false,
            },
